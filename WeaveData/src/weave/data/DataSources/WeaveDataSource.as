	...
	public class WeaveDataSource extends AbstractDataSource implements IDataRowSource
	{
		...
		override public function getColumnReference(node:IWeaveTreeNode, output:ILinkableDynamicObject):Boolean
		{
			...
			if (entityNode)
			{
				...
				var xml:XML = HierarchyUtils.nodeFromMetadata(entityNode.getEntity().publicMetadata);
		...
		private function handleGetAttributeColumnFault(event:FaultEvent, column:ProxyColumn):void
		{
			...
			var msg:String = "Error retrieving column: " + ObjectUtil.toString(column.getProxyMetadata()) + ' (' + event.fault.faultString + ')';
...
//		}
		private function handleGetAttributeColumn(event:ResultEvent, proxyColumn:ProxyColumn):void
		{
			...
			try
			{
				if (!event.result)
				{
					reportError("Did not receive any data from service for attribute column: " + ObjectUtil.toString(metadata));
					return;
				}
			...
			{
				reportError(e);
				trace(this,"handleGetAttributeColumn",ObjectUtil.toString(metadata),e.getStackTrace());
			}
		}
	}
}
